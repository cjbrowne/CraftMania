...
public class Sky extends GameObject
{
	...
	{
		PerformanceMonitor.getInstance().start(PerformanceMonitor.Operation.RENDER_SKY);
		/* Sphere */
		GL11.glDisable(GL11.GL_TEXTURE_2D);
		drawShpere(_playerPos.x(), Math.max(_cloudsHeight + _bend + 10.0f, _playerPos.y() + _height), _playerPos.z());
		GL11.glEnable(GL11.GL_TEXTURE_2D);

		PerformanceMonitor.getInstance().stop(PerformanceMonitor.Operation.RENDER_SKY);
	}

	public void renderClouds()
	{
		PerformanceMonitor.getInstance().start(PerformanceMonitor.Operation.RENDER_CLOUDS);
		...
		PerformanceMonitor.getInstance().stop(PerformanceMonitor.Operation.RENDER_CLOUDS);
	}
